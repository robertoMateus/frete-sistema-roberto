...
<div class="container">
    ...
    <form method="get" action="${pageContext.request.contextPath}/motoristas/listar" class="filtro-form">
        ...
        <c:if test="${not empty filtro}">
            <a href="${pageContext.request.contextPath}/motoristas/listar" class="btn btn-secondary">Limpar</a>
        </c:if>
    </form>
                        ...
                        <c:forEach var="motorista" items="${motoristas}">
                            <tr>
                                ...
                                <td>
                                    ...
                                    <c:if test="${motorista.dataValidadeCnh != null and now != null and motorista.dataValidadeCnh.isBefore(now)}">
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${motorista.status.name() == 'ATIVO'}">
                                            <span class="badge badge-ativo">${motorista.status.descricao}</span>
                                        </c:when>
                                        <c:when test="${motorista.status.name() == 'SUSPENSO'}">
                                            <span class="badge badge-suspenso">${motorista.status.descricao}</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="badge badge-inativo">${motorista.status.descricao}</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td>
                                    <div class="acoes">
                                        ...
                                        <c:if test="${motorista.status.name() == 'ATIVO'}">
                                            <form method="post" action="${pageContext.request.contextPath}/motoristas/inativar"
                                            ...
                                            <form method="post" action="${pageContext.request.contextPath}/motoristas/suspender"
                                                  style="display:inline"
                                        ...
                                        <form method="post" action="${pageContext.request.contextPath}/motoristas/excluir"
                                              style="display:inline"
                ...
                <c:if test="${totalPaginas > 1}">
                    <div class="paginacao">
                        <c:if test="${paginaAtual > 1}">
                            <a href="${pageContext.request.contextPath}/motoristas/listar?pagina=${paginaAtual - 1}&filtro=${filtro}">← Anterior</a>
                        </c:if>
                        ...
                        <c:if test="${paginaAtual < totalPaginas}">
                            <a href="${pageContext.request.contextPath}/motoristas/listar?pagina=${paginaAtual + 1}&filtro=${filtro}">Próxima →</a>
                        </c:if>
